        ...
        <div class="container">
            ...
            <?php elseif (current_user_can('subscriber') || current_user_can('artist')): ?>
                ...
                <?php if (current_user_can('artist')): ?>
                    <button id="createArtistPageBtn" class="btn btn-lg"
                        style="background-color: var(--light-bg); color: var(--primary-color)!important; padding: 12px 40px; border-radius: 50px; text-transform: uppercase; font-weight: bold; transition: all 0.3s ease;">
                        Create Artist Page
                    </button>
                <?php else: ?>
                ...
                <!-- Floating Form (Initially Hidden) -->
                <div id="artistFormContainer">
                    <button id="closeForm" style="
                        ...
                        font-size: 1.5rem;
                        cursor: pointer;
                        color: var(--primary-color) !important;">&times;
                    </button>
        ...
        <div class="container">
            <div class="row">
                <?php if (have_posts()): ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <p class="text-center" style="font-size: 1.2rem; color: var(--secondary-text);">Sorry, no posts were
                        found.</p>
                <?php endif; ?>
            </div>
        </div>
...
<script>
    ...
    document.getElementById('createArtistPageBtn')?.addEventListener('click', function () {
        document.getElementById('overlay').classList.add('active');
        const form = document.getElementById('artistFormContainer');
        form.style.display = 'block';
        setTimeout(() => form.classList.add('active'), 10); // Small delay for smooth transition
