...
<div class="container-fluid" style="background-color: var(--light-bg); color: var(--primary-text); font-family: 'Lato', sans-serif;">
    ...
    <div class="hero-section text-center py-5" style="background: radial-gradient(circle, rgba(220,78,119,1) 0%, rgba(142,50,87,1) 100%); color: var(--light-bg); border-top-left-radius: 10px; border-top-right-radius: 10px;">
        <h1 class="display-4" style="font-weight: bold;">All Vinyls</h1>
    </div>
    ...
    <div id="content" class="content-section py-5" style="background-color: var(--mid-bg); border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; padding-top: 8px !important;">
        ...
        <div class="text-center">
            <!-- Search Bar -->
            <input type="text" id="search-bar" class="form-control d-inline-block w-auto" placeholder="Search Vinyls..."
            ...
            <div class="d-flex flex-wrap justify-content-center gap-3" id="items-container">
                ...
                // Query to fetch vinyls based on sorting
                $vinyls_query = new WP_Query(array(
                    'post_type' => 'vinyl', // Change 'vinyl' to your custom post type slug
                    'orderby' => $orderby,
                    'order' => $order,
                    'posts_per_page' => -1 // Display all posts
                ));
                ?>
                <?php if ($vinyls_query->have_posts()): ?>
                    <?php while ($vinyls_query->have_posts()): $vinyls_query->the_post(); ?>
                        <div class="card text-center vinyl-item" data-name="<?php echo strtolower(get_the_title()); ?>"
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <p class="text-center" style="font-size: 1.2rem; color: var(--secondary-text);">No vinyls found.</p>
                <?php endif; ?>
...
<script>
    ...
    document.getElementById('search-bar').addEventListener('input', function () {
        ...
        let items = document.querySelectorAll('.vinyl-item');
